                            ...
                            <table id="datatable-main" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @foreach ($projects as $project)
                                        @php
                                            $courseCount = count($project->dosenPengampu->where('dosen.id_user', Auth::user()->id));
                                            $firstCourse = true;
                                        @endphp
                                                ...
                                                <tr>
                                                    @if ($firstCourse)
                                                        ...
                                                        <td rowspan="{{ $courseCount }}">
                                                            @foreach ($project->mahasiswa as $index => $mahasiswa)
                                                                <p>{{ $loop->iteration }}. {{ $mahasiswa->nama_lengkap }} - {{ $mahasiswa->nim }}</p>
                                                            @endforeach
                                                        </td>
                                                        <td rowspan="{{ $courseCount }}">
                                                            @php
                                                            ...
                                                            <div class="progress progress-sm">
                                                                <div class="progress-bar bg-success" role="progressbar"
                                                                    aria-valuenow="{{ $project->overallProgress }}" aria-valuemin="0" aria-valuemax="100"
                                                    ...
                                                    <td>
                                                        <button type="button" class="btn btn-block btn-sm btn-outline-info">
                                                            <a href="{{ route('nilai-mingguan.custom_edit', ['project_id' => $project->project_id, 'kode_matakuliah' => $dosenPengampu->kode_mata_kuliah]) }}">Masukkan Nilai</a>
                                                        </button>
                                                    </td>
                                                </tr>
                                            @endif
